...
public:
	...
	bool operator < (const CChineseNode &p) {
		if (this->m_nMinSplit == 1 && p.m_nMinSplit != 1) {
			return 1;
		}
		if (this->m_nMinSplit != 1 && p.m_nMinSplit == 1) {
			return 0;
		}
		if (this->m_nMinProb < p.m_nMinProb) {
			return 1;
		}
		return 0;
	}
...
public:
	...
	void InputJoint(CInputString* pCurString, CInputString* pLastInput, set<char*> chinese){
		...
		for (int k = 0; k < vecChineseBuff.size(); k++) {
			pCurString->FastJudege(vecChineseBuff[k]);
			pCurString->InsertChineseOld(vecChineseBuff[k]);
		}
